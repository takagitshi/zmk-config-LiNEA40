...
struct gesture_processor_data {
    struct k_mutex lock;
    struct linea40_gesture_state states[ZMK_INPUT_LISTENERS_LEN];
};
...
static void reset_all_states(const struct device *dev) {
    ...
    for (size_t i = 0; i < ARRAY_SIZE(data->states); i++) {
        linea40_gesture_state_reset(&data->states[i]);
    }
    k_mutex_unlock(&data->lock);
}

static void invoke_action(const struct gesture_processor_config *cfg,
                          enum linea40_gesture_direction direction, uint8_t listener_index) {
    uint16_t position;
    switch (direction) {
    case LINEA40_GESTURE_UP:
        position = cfg->up_position;
        break;
    case LINEA40_GESTURE_LEFT:
        position = cfg->left_position;
        break;
    case LINEA40_GESTURE_RIGHT:
        position = cfg->right_position;
        break;
    case LINEA40_GESTURE_DOWN:
...
static int gesture_processor_handle_event(const struct device *dev, struct input_event *event,
    ...
    if (!zmk_keymap_layer_active(cfg->layer)) {
        linea40_gesture_state_reset(&data->states[processor_state->input_device_index]);
        k_mutex_unlock(&data->lock);
        return ZMK_INPUT_PROC_CONTINUE;
    }

    const enum linea40_gesture_axis axis =
        event->code == INPUT_REL_X ? LINEA40_GESTURE_AXIS_X : LINEA40_GESTURE_AXIS_Y;
    enum linea40_gesture_direction direction =
        linea40_gesture_state_update(&data->states[processor_state->input_device_index], axis,
                                  event->value, event->sync, k_uptime_get(), cfg->threshold,
                                  cfg->cooldown_ms);
    event->value = 0;
    k_mutex_unlock(&data->lock);

    if (direction != LINEA40_GESTURE_NONE && zmk_keymap_layer_active(cfg->layer)) {
